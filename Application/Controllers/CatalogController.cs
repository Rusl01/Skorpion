...
public class CatalogController : Controller
{
    ...
    public IActionResult Index(string sortOrder, string searchString, CatalogViewModel catalogViewModel)
    {
        ...
        {
            var gameGenres = _db.Genres.Where(genre => genre.Games.Any(j => j.GameId == games[i].Id)).ToList();
            var gamePlatforms = _db.Platforms.Where(platform => platform.Games.Any(j => j.GameId == games[i].Id))
                .ToList();
            var gamePlayers = _db.Players.Where(player => player.Games.Any(j => j.GameId == games[i].Id)).ToList();
