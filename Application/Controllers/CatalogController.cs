    ...
    public class CatalogController : Controller
    {
        ...
        public IActionResult Index(string sortOrder, string searchString, CatalogViewModel catalogViewModel)
        {
            ...
            for (var i = 0; i < games.Count; i++)
            {
                ...
                var gamePlatforms = db.Platforms.Where(platform => platform.Games.Any(j => j.GameId == games[i].Id)).ToList();
                ...
                foreach (var t in gamePlatforms.Where(t => !selectedPlatforms.Contains(t.Name)))
                {
                    newGames.Remove(games[i]);
                }
                foreach (var t in gamePlayers.Where(t => !selectedPlayers.Contains(t.Name)))
                {
                    newGames.Remove(games[i]);
                }
            }
